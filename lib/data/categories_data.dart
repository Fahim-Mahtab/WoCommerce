class CategoriesData {
  static final List<Map<String, String>> categories = [
    {
      'imageUrl':
      'https://static.vecteezy.com/system/resources/thumbnails/005/346/410/small_2x/close-up-portrait-of-smiling-handsome-young-caucasian-man-face-looking-at-camera-on-isolated-light-gray-studio-background-photo.jpg',
      'items': 'Men',
    },
    {
      'imageUrl':
          'https://img.freepik.com/premium-photo/beautiful-young-woman-portrait-close-up-outdoor_1024630-5348.jpg',
      'items': 'Women',
    },
    {
      'imageUrl':
          'https://media.istockphoto.com/id/1399611777/photo/portrait-of-a-smiling-little-brown-haired-boy-looking-at-the-camera-happy-kid-with-good.jpg?s=612x612&w=0&k=20&c=qZ63xODwrnc81wKK0dwc3tOEf2lghkQQKmotbF11q7Q=',
      'items': 'Kids',
    },
    {
      'imageUrl':
          'https://ddfndelma2gpn.cloudfront.net/card_image/None/Sony_WH-CH510_Wireless_Headphones.webp',
      'items': 'Headphone',
    },
    {
      'imageUrl':
          'https://m.media-amazon.com/images/I/8112+cxep9L._AC_SL1500_.jpg',
      'items': 'Bags',
    },
    {
      'imageUrl':
          'https://m.media-amazon.com/images/I/71gbYy1I2JL._AC_SL1500_.jpg',
      'items': 'Jacket',
    },

  ];
}
